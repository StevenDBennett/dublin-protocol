...
public:
    ...
    void sequential_carry(std::vector<uint64_t>& data) {
        uint64_t carry = 0;
    ...
    void comprehensive_benchmark() {
        ...
        std::cout << "2. Cache-Aware Optimization:\n";
        std::cout << "   Time: " << time2 << " ms\n";
        std::cout << "   Throughput: " << (data_size / time2 * 1000) << " carries/sec\n\n";

        // Test 3: Sequential
        std::vector<uint64_t> seq_data = test_data;
        start = std::chrono::high_resolution_clock::now();
        sequential_carry(seq_data);
        end = std::chrono::high_resolution_clock::now();
        double time3 = std::chrono::duration<double, std::milli>(end - start).count();

        std::cout << "3. Sequential Optimization:\n";
        std::cout << "   Time: " << time3 << " ms\n";
        std::cout << "   Throughput: " << (data_size / time3 * 1000) << " carries/sec\n\n";

        // Performance summary
        std::cout << "=== PERFORMANCE SUMMARY ===\n";
        double best_time = std::min({time1, time2, time3});
        std::cout << "Best throughput: " << (data_size / best_time * 1000) << " carries/sec\n";
        std::cout << "That's over " << (data_size / best_time * 1000 / 1000000.0) << " million carries per second!\n";
